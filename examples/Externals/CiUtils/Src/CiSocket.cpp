...
CCiSocket::CCiSocket(CCiSocketType_t socketType, int iSendBufferSize, int iRecvBufferSize)
{
	...
	m_pSendBuffer = static_cast<char*>(malloc(m_iSendBufferSize));
	m_pRecvBuffer = static_cast<char*>(malloc(m_iRecvBufferSize));
}
...
bool CCiSocket::WriteInt16WithTimeOut(short sInt16)
{
	short int16_temp;
	int nwritten;

	int16_temp = htons(sInt16);
	if ( !WriteNWithTimeOut((const void *)&int16_temp, 2, &nwritten) )
	{
		return false;
	}
...
bool CCiSocket::WriteInt32WithTimeOut(int iInt32)
{
	int int32_temp;
	int nwritten;

	int32_temp = htonl(iInt32);
	if ( WriteNWithTimeOut((const void *)&int32_temp, 4, &nwritten) != true )
	{
		return false;
	}
...
bool CCiSocket::WriteInt64WithTimeOut(long long llInt64)
{
	...
	if ( WriteNWithTimeOut((const void *)&int64_temp, 8, &nwritten) != true )
	{
		return false;
	}
...
bool CCiSocket::WriteNWithTimeOut(const void* pBuf, int ntowrite, int* pnwritten)
{
	int nleft;
	int nwritten_temp;
	const char *ptr;
	...
	{
#ifdef _WIN32
		struct timeval timeout = { m_iSendTimeOutMillieSec/1000, (m_iSendTimeOutMillieSec% 1000)*1000 };
		...
		int iNEvent = select( m_iSocket+1, NULL, &wset, NULL, &timeout);
		...
		do
		{
			iNEvent = poll(&pollfd, 1, m_iSendTimeOutMillieSec);
		}while(iNEvent < 0 && errno == EINTR);
...
bool CCiSocket::ReadInt16WithTimeOut(short* psInt16)
{
	short int16_temp;
	int nread;

	if ( !ReadNWithTimeOut((void *)&int16_temp, 2, &nread) )
	{
		return false;
	}
...
bool CCiSocket::ReadInt32WithTimeOut(int* piInt32)
{
	int int32_temp;
	int nread;

	if ( ReadNWithTimeOut((void *)&int32_temp, 4, &nread) != true )
	{
		return false;
	}
...
bool CCiSocket::ReadInt64WithTimeOut(long long* pllInt64)
{
	...
	if ( ReadNWithTimeOut((void *)&int64_temp0, 8, &nread) != true )
	{
		return false;
	}
...
bool CCiSocket::ReadNWithTimeOut(void* pBuf, int ntoread, int* pnread)
{
	int nleft;
	int nread_temp;
	char *ptr;
	...
	while ( nleft > 0 ) {
#ifdef _WIN32
		struct timeval timeout = { m_iRecvTimeOutMillieSec/1000, (m_iRecvTimeOutMillieSec% 1000)*1000 };
		...
		int iNEvent = select( m_iSocket+1, &rset, NULL, NULL, &timeout);
		...
		do
		{
			iNEvent = poll(&pollfd, 1, m_iRecvTimeOutMillieSec);
		}while(iNEvent < 0 && errno == EINTR);
...
bool CCiSocket::ReadPatternWithTimeOut(void* pBuf, const char *pattern, int readsize, int* pnread)
{
	int nleft;
	int nread_temp;
	char *ptr, *ptrPattern = NULL;
	...
	while ( nleft > 0 ) {
		...
		/* found */
		if ( (ptrPattern = strstr((char *)pBuf, pattern)) != NULL )
			break;
	}

	*pnread = readsize - nleft;
	/* found */
	return ( ptrPattern != NULL );
}
...
{
	if (!PrepareRecvBuffer(static_cast<size_t>(iReadSize))) return false;
...
bool CCiSocket::PrepareRecvBuffer(size_t size)
{
	if (size < RemainingRecvBufferSize()) return true;
	size_t need_size = size - RemainingRecvBufferSize() + 1;
	size_t new_buffer_size = m_iRecvBufferSize + need_size ;
	void* new_buffer = realloc(m_pRecvBuffer, new_buffer_size);
	if (!new_buffer) return false;

	// new buffer is ready
	m_iRecvBufferSize = static_cast<int>(new_buffer_size);
	m_pRecvBuffer = static_cast<char*>(new_buffer);
	if (m_iRecvEndPtr < m_iRecvStartPtr)
	{
		memmove(m_pRecvBuffer + m_iRecvStartPtr + need_size, // dest
				 m_pRecvBuffer + m_iRecvStartPtr, // src
				 need_size); // size
		m_iRecvStartPtr += need_size;
	}
	return true;
}

size_t CCiSocket::RemainingRecvBufferSize()
{
	return static_cast<size_t>(m_iRecvBufferSize - GetCurrentRecvDataSize());
}
...
bool CCiSocket::WriteNFlush(const void* pBuf, int ntowrite, int* pnwritten)
{
	int nleft;
	int nwritten_temp;
	const char *ptr;
	...
	while ( nleft > 0 )
	{
...
#endif
		{
			...
			if ( iCount > 3000 )
			{
				errno = CI_ETIMEDOUT;			/* 2004.06.21 NURI. application에게 제공 */
				return false;			/* 2004.06.18 NURI */
			}
		}
		else
		{
...
{
	if (!PrepareSendBuffer(static_cast<size_t>(ntowrite))) return false;
...
bool CCiSocket::PrepareSendBuffer(size_t size)
{
	if (size < RemainingSendBufferSize()) return true;
	size_t need_size = size - RemainingSendBufferSize() + 1;
	size_t new_buffer_size = m_iSendBufferSize  + need_size ;
	void* new_buffer = realloc(m_pSendBuffer, new_buffer_size);
	if (!new_buffer) return false;

	//new buffer is ready
	m_iSendBufferSize = static_cast<int>(new_buffer_size);
	m_pSendBuffer = static_cast<char*>(new_buffer);
	if (m_iSendEndPtr < m_iSendStartPtr)
	{
		memmove(m_pSendBuffer + m_iSendStartPtr + need_size, // dest
				 m_pSendBuffer + m_iSendStartPtr, // src
				 need_size); // size
		m_iSendStartPtr += need_size;
	}
	return true;
}

size_t CCiSocket::RemainingSendBufferSize()
{
	return static_cast<size_t>(m_iSendBufferSize - GetRemainSendDataSize());
}

bool CCiSocket::WriteFromBufferToNetwork()
{
	if ( GetRemainSendDataSize() == 0 ) return true;
...
bool CCiSocket::GetBufferClon(CCiSocket &rhs)
{
	if ( m_iSendBufferSize != rhs.m_iSendBufferSize || m_iRecvBufferSize != rhs.m_iRecvBufferSize )
	{
		return false;
	}

	memcpy(m_pSendBuffer, rhs.m_pSendBuffer, m_iSendBufferSize);
	memcpy(m_pRecvBuffer, rhs.m_pRecvBuffer, m_iRecvBufferSize);

	m_iSendStartPtr = rhs.m_iSendStartPtr;
	m_iSendEndPtr = rhs.m_iSendEndPtr;
	m_iRecvStartPtr = rhs.m_iRecvStartPtr;
	m_iRecvEndPtr = rhs.m_iRecvEndPtr;

	return true;

}

int CCiSocket::StrExistInReadBuffer(const char *pStr)
{
	if ( m_pRecvBuffer == NULL || pStr == NULL ) return -1;

	int iOffset = -1;
	if ( m_iRecvStartPtr < m_iRecvEndPtr )
	{
		m_pRecvBuffer[m_iRecvEndPtr] = '\0';
		char *pStrMatch = strstr(m_pRecvBuffer + m_iRecvStartPtr, pStr);
		if ( pStrMatch != NULL )
			iOffset = static_cast<int>(pStrMatch - ( m_pRecvBuffer + m_iRecvStartPtr));
		else
			iOffset = -1;
	}
	else if ( m_iRecvStartPtr > m_iRecvEndPtr )
	{
		int iTempbufferSize = (m_iRecvBufferSize - m_iRecvStartPtr) + m_iRecvEndPtr + 1;
		char *pszTempBuffer = new char[iTempbufferSize];

		memcpy(pszTempBuffer, m_pRecvBuffer + m_iRecvStartPtr, m_iRecvBufferSize - m_iRecvStartPtr );
		memcpy(pszTempBuffer + (m_iRecvBufferSize - m_iRecvStartPtr), m_pRecvBuffer, m_iRecvEndPtr);

		pszTempBuffer[(m_iRecvBufferSize - m_iRecvStartPtr) + m_iRecvEndPtr] = '\0';

		char *pStrMatch = strstr(pszTempBuffer, pStr);
		if ( pStrMatch != NULL )
			iOffset = static_cast<int>(pStrMatch - pszTempBuffer);
		else
			iOffset = -1;
